...
if ( ! defined( 'ABSPATH' ) ) exit;
             ...
             public function plugins_api_call($def, $action, $args)
                 {
                     ...
                     if($request_string === FALSE)
                        return new WP_Error('plugins_api_failed', __('An error occour when try to identify the pluguin.' , 'divi-machine') . '&lt;/p> &lt;p>&lt;a href=&quot;?&quot; onclick=&quot;document.location.reload(); return false;&quot;>'. __( 'Try again', 'divi-machine' ) .'&lt;/a>');;

                     $request_uri = $this->api_url . '?' . http_build_query( $request_string , '', '&');
                     $data = wp_remote_get( $request_uri );

                     if(is_wp_error( $data ) || $data['response']['code'] != 200)
                        return new WP_Error('plugins_api_failed', __('An Unexpected HTTP Error occurred during the API request.' , 'divi-machine') . '&lt;/p> &lt;p>&lt;a href=&quot;?&quot; onclick=&quot;document.location.reload(); return false;&quot;>'. __( 'Try again', 'divi-machine' ) .'&lt;/a>', $data->get_error_message());
    ...
    function DE_DMACH_run_updater()
         {

             $wp_plugin_auto_update = new DE_DMACH_CodeAutoUpdate(DE_DMACH_APP_API_URL, 'divi-machine', 'divi-machine/divi-machine.php');
    ...
    add_action( 'after_setup_theme', 'DE_DMACH_run_updater' );
